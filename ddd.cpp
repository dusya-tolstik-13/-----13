...
int kolvodel(string num) {
    ...
    while (num != "1") { // Пока число не будет сведено к 1
        ...
        if (!divided) {
            // Если число не делится на простые числа в текущем диапазоне,
            // и мы не смогли его разложить, это значит, что оно простое
            if (!isDivided) {
                nacdel = 2; // простое число делится на 1 и на само себя
            }
